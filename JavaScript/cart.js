let cart = [];
let products = [];
const DELIVERY = 150; // flat delivery fee in ZAR
...
function addCartToHTML() {
    ...
    cart.forEach((product, index) => {
        let price = Currency.toBase(product.price);
        let lineTotal = price * product.quantity;
        content += `
            ...
            <div class="cart_product_info">  
                <div class="top_card">
                    <div class="left_card">
                        ...
                        <span class="product_price">${Currency.dual(product.price)}</span>
                    </div>
                ...
                <div class="buttom_card">
                    ...
                    <span class="total_price">${Currency.dual(lineTotal)}</span>
                </div>
            </div>
        </div>`;
    });
...
function updateTotalPrice() {
    ...
    totalPrice.innerHTML = Currency.dual(total);
    localStorage.setItem("total price" , total + DELIVERY);
    return total;
}
...
function checkCartPage(total,totalQuantity){
    if (window.location.pathname.includes("cartPage.html")) {
        document.getElementById("Delivery").innerHTML = Currency.dual(DELIVERY);
        if (cart.length == 0) {
            cartItemsCount.innerHTML = `(0 items)`;
            document.getElementById("Subtotal").innerHTML = Currency.dual(0);
            document.getElementById("total_order").innerHTML = Currency.dual(0);
        }
        else{
...
function displayInCartPage(total){
    ...
    subTotal.innerHTML = Currency.dual(total);
    let totalOrder = total + DELIVERY;
    document.getElementById("total_order").innerHTML = Currency.dual(totalOrder);
}
function checkOut(){
